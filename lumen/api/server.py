...
class LumenServer:
    ...
    def _profile(self) -> Dict[str, Any]:
        return {
            ...
            "version": VERSION,
            "tagline": TAGLINE,
            "concept": "Самостоятельная ИИ-платформа, полностью на собственном "
                       "ИИ: единственный генеративный модуль — LUMEN Core "
                       "(встроенная база знаний, диалоговая память, "
                       "планирование, инструменты) — офлайн, без внешних "
                       "API и ключей. Конвейер «Луч»: приём → защита → "
                       "анализ → контекст → инструменты → генерация.",
            "stages": ["Приём", "Защита", "Анализ", "Контекст", "Инструменты",
                       "Генерация"],
            "memory_layers": ["working (сессия)", "episodic (журналы)",
                              "semantic (факты)"],
            "backends": ["LUMEN Core — собственный ИИ (офлайн, без API) — "
                         "единственный модуль"],
