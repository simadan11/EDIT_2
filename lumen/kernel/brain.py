...
самостоятельный локальный разум, работающий на чистом Python:
  ...
  • честность: если запроса нет в ядре — говорит, чего не хватает,
    и (опционально) отдаёт свободный текст внешнему генеративному
    модулю (Gemini / OpenAI-совместимый), если он подключён.
...
class LumenCore:
    ...
    def __init__(self, language: str = "ru",
        ...
        self._thanks_i = 0
        self._fb_i = 0
        # сработал ли честный фолбэк (сигнал overflow для внешних модулей)
    ...
    def quick_answer(self, text: str,
        ...
        Возвращает (ответ, нужен_внешний_модуль) — сигнал overflow для
        гибридного режима (см. backends.py / core/model_router.py).
        """
    ...
    def _answer(self, *, text: str, plan: Dict[str, Any],
        ...
        if primary == "identity":
            return self._variant(text, [
                ...
                f"Мой основной разум — {MODEL_NAME.replace('-1', ' Core')}: собственный локальный "
                f"модуль со встроенной базой знаний, диалоговой памятью и инструментами — "
                f"без облака. Каждый запрос проходит конвейер «{PIPELINE_NAME}» из шести стадий. "
                f"Внешние LLM-модули (Gemini, OpenAI-совместимые) — опциональная надстройка, "
                f"а не основа.",
                f"Я {BRAND_NAME}. Основа — собственное ядро {MODEL_NAME} с модулем "
                f"{MODEL_NAME.replace('-1', ' Core')}: база знаний, память, планирование и "
                f"инструменты живут на вашей машине и работают офлайн. "
                f"{TAGLINE} — не маркетинг, а принцип работы.",
            ])
        ...
        if primary == "capability":
            lines = [
                ...
                "• Свободные творческие тексты: если подключён внешний модуль "
                "(Gemini / Ollama) — делегирую ему, иначе честно скажу, что без него.",
            ]
            ...
            if primary == "greeting":
                ...
                if hits:
                    ...
                    parts.append(self._variant(text, [
                        ...
                        "Для свободных творческих текстов подключите внешний модуль в Настройках.",
                    ]))
