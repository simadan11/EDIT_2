...
## LUMEN 1.1 — «Собственный ИИ: LUMEN Core»

Основной ИИ платформы — теперь **LUMEN Core**, собственный локальный
модуль. Gemini и OpenAI-совместимые серверы больше не «мозг» — они
опциональные внешние надстройки.

### Добавлено

- **`lumen/kernel/brain.py` — LUMEN Core**: собственный ИИ-модуль
  (основной генератор стадии 6):
  ...
  - календарная математика («сколько дней до 31 декабря?», «через 30
    дней»), текстовая статистика (слова/знаки в цитате);
  - намерения `knowledge`, `joke`, `riddle`, `date_math`, `text_ops`;
  - детерминированная вариативность формулировок; честный ответ,
    когда запроса нет в ядре (сигнал overflow).
- **Гибридный режим (overflow)** — `backend.overflow_provider`:
  LUMEN Core обрабатывает всё структурированное, свободный творческий
  текст — подключённый внешний модуль (с пометкой в ответе).
- **`lumen_core` — основной `backend.provider`** (по умолчанию).
  UI: «LUMEN Core — собственный ИИ (офлайн, без ключей)» первым
  пунктом + отдельный выбор overflow-модуля.
- **`core/model_router.py` (legacy)** — по умолчанию маршрутизирует
  в LUMEN Core; Gemini — только overflow/явная модель.
...
| Было (1.0) | Стало (1.1) |
|---|---|
| Основной модуль: `heuristic` (LHC) — шаблонный генератор | `lumen_core` (LUMEN Core) — собственный ИИ с базой знаний, диалогом и вычислениями |
| Gemini / OpenAI — «генеративные модули наравне» | Внешние опциональные модули (provider или overflow) |
...
- Legacy-десктоп: голосовой режим (стриминговый протокол) без
  изменений — это протокол передачи голоса, а не «модель».
